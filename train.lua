...
function scene:create( event )
    ...
    x1={200,500,350,390,420,440,410,350,440}
    y1={280,280,410,360,500,550,580,660,550}
    x2={600,500,390,420,440,410,350,210,500}
    y2={280,760,360,500,550,580,660,380,650}
    
    --local ball3 = display.newImageRect( sceneGroup, "images/ball.jpg", 80, 80 )
    --ball3.x=440
    --ball3.y=550

    
    
    local ball1 = display.newImageRect( sceneGroup, "images/ball.jpg", 80, 80 )
    ball1.x=x1[index]
    ball1.y=y1[index]
    
    

    local ball2 = display.newImageRect( sceneGroup, "images/ball.jpg", 80, 80 )
    ball2.x=x2[index]
    ball2.y=y2[index]
    
    local function ballChange()
        ball1.x=x1[index]
        ball1.y=y1[index]
        ball2.x=x2[index]
        ball2.y=y2[index]
    ...
    function paint(event)
        ...
        elseif event.phase == "moved" then   --during touch movement
            if lineCreated then
                ...
                print (initX, initY, locationX, locationY)
                if(math.abs(initX-ball1.x)<40 and math.abs(initY-ball1.y)<40 and math.abs(locationX-ball2.x)<40 and math.abs(locationY-ball2.y)<40) then
                    print "entered"
                    
                    line = display.newLine(ball1.x, ball1.y, ball2.x, ball2.y)
                    index=index+1
                    ...
                    --correctTrack = audio.loadStream( "audio/Correct.mp3" )
                    --audio.play( correctTrack, { channel=1, loops=0 } )
                    if(table.getn(x1)<index) then
